...
public class WordStatChecker extends MainFilesChecker {
    ...
    public static final String ALL = Randomized.RUSSIAN + Randomized.ENGLISH + Randomized.GREEK + DASH;
    ...
    public String[] generateWords(final int wordLength, final int totalWords, final String chars) {
        ...
        for (int i = 0; i < totalWords; i++) {
            words[i] = randomString(allChars, wordLength / 2, wordLength);
        }
        return words;
    }
    ...
    public String[][] generateTest(final int lines, final String[] words, final int wordsPerLine) {
        ...
        for (int i = 0; i < text.length; i++) {
            text[i] = new String[randomInt(wordsPerLine / 2, wordsPerLine)];
            for (int j = 0; j < text[i].length; j++) {
                text[i][j] = randomItem(words);
            }
        }
        return text;
    }
