...
public class Scanner {
	...
	private boolean hasRead() throws IOException { //есть считанный символ
		if (!hasChar) {
			int ch = reader.read();
			if (ch == -1) {
				sym = 0;
	...
	private boolean isCorrect() throws IOException { //проверка для слов в строке
		hasRead();
		return Character.DASH_PUNCTUATION == Character.getType(sym) ||
				Character.isLetter(sym) ||
				sym == '\'' ||
				Character.isDigit(sym) ||
				!Character.isWhitespace(sym);
	} 
